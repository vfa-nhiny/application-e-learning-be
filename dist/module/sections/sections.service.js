...
let SectionsService = class SectionsService {
    ...
    async createNewLessonWithoutOrder(lessonDto) {
        const sectionDto = await this.sectionModel.findOne({ sectionId: lessonDto.sectionId }).exec();
        if (!sectionDto)
            throw new common_1.HttpException("Section not found", common_1.HttpStatus.NOT_FOUND);
        sectionDto.lessons.push(Object.assign({ lessonId: crypto.randomUUID() }, lessonDto));
        console.log(sectionDto.lessons);
        await sectionDto.save();
        return sectionDto;
    }
    async updateLesson(lessonDto) {
        ...
        if (index) {
            ...
            sectionDto.lessons[index].date = constants_1.localCurrentTime;
        }
        await sectionDto.save();
        return sectionDto;
    }
